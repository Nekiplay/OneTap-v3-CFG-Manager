UI.AddSliderInt('Bomb timer x', 0, Render.GetScreenSize()[0]); UI.SetValue('Script items', 'Bomb timer x', 250), UI.SetEnabled('Script items', 'Bomb timer x', 0); 
UI.AddSliderInt('Bomb timer y', 0, Render.GetScreenSize()[1]); UI.SetValue('Script items', 'Bomb timer y', 300), UI.SetEnabled('Script items', 'Bomb timer y', 0); 

UI.AddDropdown("Theme", ["Black", "White"])

var bomb_site,kb,xo,yo;
function render_main() {
    x = UI.GetValue('Script items', 'Bomb timer x')
    y = UI.GetValue('Script items', 'Bomb timer y')
    bomb_timer_background = UI.GetValue('Script items', 'Theme') == 0 ? [13, 13, 13, 210] : [245, 245, 245, 210];
    bomb_timer_text = UI.GetValue('Script items', 'Theme') == 1 ? [0, 0, 0, 255] : [240, 240, 240, 255];
    bomb_timer_text_2 = UI.GetValue('Script items', 'Theme') == 1 ? [48, 223, 255, 255] : [109, 232, 255, 255];

    var font_icon = Render.AddFont("bomb", 18, 0);
    //Vars
    ...
    if(c4 != undefined) {
        ...
        if(timer >= 0.1) {
            ...
            Render.StringCustom(x + 2, y + 7, 0, "W", bomb_timer_text, font_icon); //Bomb icon
            Render.StringCustom(x + add_value, y + 15, 0, timer_1.toFixed(1), bomb_timer_text, Render.AddFont("Museo Sans Cyrl 700", 8, 0)); //Bomb time
            Render.StringCustom(x + 30, y + 4, 0, "Site: ", bomb_timer_text, Render.AddFont("Museo Sans Cyrl 700", 10, 0)); //Site
            Render.StringCustom(x + 62, y + 5, 1, bomb_site, bomb_timer_text_2, Render.AddFont("Museo Sans Cyrl 700", 9, 0)); //Site place
            Render.StringCustom(x + 30, y + 20, 0, "Damage: ", bomb_timer_text, Render.AddFont("Museo Sans Cyrl 700", 10, 0)); //Damage
            Render.StringCustom(x + 100, y + 21, 1, "" + dmg, bomb_timer_text_2, Render.AddFont("Museo Sans Cyrl 700", 9, 0)); //Damage value
    ...
    /*
     если 2 цифры то : x + 140, y + 15, 0, "40.0", ||
     если 1 цифра то : x + 144, y + 15, 0, "1.0", ||
    */
    if(UI.IsMenuOpen() && Input.IsKeyPressed(0x1)) {
        ...
        if (in_bounds(mousep, x, y, x + 200, y + 50)) {
            ...
            UI.SetValue("Script items", "Bomb timer x", mousep[0] - xo)
            UI.SetValue("Script items", "Bomb timer y", mousep[1] - yo)
        }
